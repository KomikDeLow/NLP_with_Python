...
word_features = all_words.keys()[:300] #Limit the number of features that the classifier needs to 
                                       #process (200 most frequent words in the overall corpus)
def document_features(document): #Define a feature extractor for documents, to help the classifier to 
                                 #identify which aspects of the data it should pay attention to
    document_words = set(document) #Check whether a word occurs in a set 
	                               #(it's much faster than checking whether it occurs in a list)
    features = {}
