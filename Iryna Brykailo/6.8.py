...
documents = [(list(movie_reviews.words(fileid)), category) # Creating a randomized list for each fileid
...
print len(all_words)
word_features = all_words.keys()[:200] # Extracting the features
def document_features(document):
    document_words = set(document)
    features = {}
    for word in word_features:
        features['contains(%s)' % word] = (word in document_words)
    return features            # Calling the function
...
print len(word_features)             # Printing the length
featuresets = [(document_features(d), c) for (d,c) in documents]
train_set, test_set = featuresets[90:], featuresets[:30]
classifier = nltk.NaiveBayesClassifier.train(train_set)
print nltk.classify.accuracy(classifier, test_set)  # Defining the accuracy 
