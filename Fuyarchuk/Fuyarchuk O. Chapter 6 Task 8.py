...
print nltk.classify.accuracy(classifier, test_set) #ќтримую базову точн≥сть
#якщо слово не попало в список найчастотн≥ших але його г≥перн≥ми вход€ть у цей список то:
#додати це слово до списку найчастотн≥ших
...
#—к≥льки сл≥в додалос€ до списку
print len(word_features)
featuresets = [(document_features(d), c) for (d,c) in documents]
train_set, test_set = featuresets[80:], featuresets[:20]
classifier = nltk.NaiveBayesClassifier.train(train_set)
print nltk.classify.accuracy(classifier, test_set)
 #—л≥в додалос€ 238 а точн≥сть так ≥ не зб≥льшилась.
